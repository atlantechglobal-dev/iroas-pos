 ...
 */
export async function qrDataUrl(value, { width = 300, margin = 4 } = {}) {
  if (!value) return ''
  const toDataURL = QRCode?.toDataURL || QRCode?.default?.toDataURL
  if (typeof toDataURL !== 'function') {
    throw new Error('QR library unavailable')
  }
  // Generate sharper modules + quiet zone so phone cameras can decode reliably.
  const pixelWidth = Math.max(Number(width) || 300, 256)
  return toDataURL.call(QRCode.default || QRCode, String(value), {
    width: pixelWidth,
    margin: Math.max(2, Number(margin) || 4),
...
export function QrCodePreview({
  ...
  useEffect(() => {
    ...
    qrDataUrl(value, { width: Math.max(size * 3, 320), margin: 4 })
      .then((dataUrl) => {
        if (!cancelled) {
          setSrc(dataUrl)
          setError('')
  ...
  return (
    <div
      ...
      style={{
        ...
        height: size,
        borderRadius: 8,
        overflow: blurred ? 'hidden' : 'visible',
        background: '#fff',
        ...style,
      }}
    >
