import '@/shared/ui/businessCard/BusinessCardVisual.css'
...
function usableImageSrc(value) {
  if (!value) return ''
  if (typeof value === 'object' && value.dataUrl) return usableImageSrc(value.dataUrl)
...
function bgImageStyle(value) {
  const src = usableImageSrc(value)
  return src ? { backgroundImage: `url("${src}")` } : undefined
}

function LogoMark({ logoDataUrl, name }) {
  const src = usableImageSrc(logoDataUrl)
  if (src) {
    return <img className="bcv-logo-img" src={src} alt="" />
  }
...
function HeroSurface({ heroDataUrl, className = '' }) {
  const src = usableImageSrc(heroDataUrl)
  return (
    <div
  ...
  return (
    <div className="bcv-inner layout-split-gold">
      ...
      <div
        ...
        style={bgImageStyle(card.circleDataUrl) || bgImageStyle(card.heroDataUrl)}
        aria-hidden="true"
      />
    </div>
  )
  ...
  return (
    <div className="bcv-inner layout-noir-orange">
      ...
      <div className="bcv-panel bcv-photo-side">
        ...
        <HeroSurface heroDataUrl={card.heroDataUrl} />
        {usableImageSrc(card.circleDataUrl) ? (
          <div
            className="bcv-circle sm"
            style={bgImageStyle(card.circleDataUrl)}
            aria-hidden="true"
          />
        ) : null}
      </div>
